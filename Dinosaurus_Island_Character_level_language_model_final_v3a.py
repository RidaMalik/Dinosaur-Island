...
def sample(parameters, char_to_ix, seed):
    ...
    n_a = Waa.shape[1]
    #print("vocab_size:", str(vocab_size)) # 27
    # VOCAB_SIZE variable contains the number of unique character in the text corpus.
    # There are 19909 total characters and 27 unique characters in your data.
    #print("Waa.shape:", str(Waa.shape)) # (100,100)
    #print("n_a:", str(n_a)) # 100
...
def model(data, ix_to_char, char_to_ix, num_iterations = 35000, n_a = 50, dino_names = 7, vocab_size = 27):
    ...
    n_x, n_y = vocab_size, vocab_size #27,27
    
    # Initialize parameters
    parameters = initialize_parameters(n_a, n_x, n_y) # <Gema> in utils.py, it returns Wax,Waa,Wya,b,by
    
    # Initialize loss (this is required because we want to smooth our loss, don't worry about it)
    loss = get_initial_loss(vocab_size, dino_names) # <Gema> in utils.py
    ...
    for j in range(num_iterations):
        ...
        X = [None] + [char_to_ix[ch] for ch in examples[index]] #inputs to the RNN
        Y = X[1:] + [char_to_ix["\n"]] #the targets it should be outputting
